...
// NSLog(@"tp: %f; xanim: %f", transitionProgress, xanim);
        ...
        CGFloat alphaRecording = 1.0f;
        CGFloat alphaView = 1.0f;
        if (isAnimatingNow && !isAnimatingTowardsRecording) {
            alphaRecording = 1.0f - xanim;
            alphaView = xanim;
        }
        if (isAnimatingNow && isAnimatingTowardsRecording) {
            alphaView = 1.0f - xanim;
...
- (void)_loadKeyCombo {
    ...
    if ((defaultsKey != nil) && [defaultsKey length]) {
        ...
        NSUInteger flags;
        if ((nil == [savedCombo valueForKey:@"modifierFlags"]) && (nil != [savedCombo valueForKey:@"modifiers"])) {
            // carbon, for compatibility with PTKeyCombo
